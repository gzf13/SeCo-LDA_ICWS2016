...
contextS = SW(ssum,:);
serviceNum = length(ssum);

%%
% 将文档-单词关系写入到文件中去
fid = fopen('step5_0_wordcount_scontext.txt','wt');
for i=1:1:serviceNum
    %Mashup i
    temIndex = find(contextS(i,:));
...
for t_i = 1:length(T_N)
   ...
   disp('1. 开始LDA模型计算...');
   % LDA模型
   [WS, DS] = importworddoccounts('step5_0_wordcount_scontext.txt');    %利用importworddoccounts读入所需参数
    ...
    temLValue = 0;
    for i=1:serviceNum
       % each doucment(service)
       word_index = find(contextS(i,:));  %找出di包含的所有单词
    ...
    sigma_Nd = sum(sum(contextS));
    perplexity(t_i) = exp(-sigma_log_pwd/sigma_Nd);
end


save STEP5_0_scontext minD_JS perplexity;
%%
% % 绘制曲线
% plot(T_N,LValue,'-b*');
% [~,index] = max(LValue);
