...
%%
P4 = zeros(leftServiceNum,leftServiceNum);
for i=1:leftServiceNum
    p_k_sl = P1_S_T(leftServiceSet(i),:);   %1*35
    for j=1:leftServiceNum
        if (i~=j)
            index = find(SW(leftServiceSet(j),:));
            for k=1:length(index)
                P4(i,j) = P4(i,j) + P1_T_W(index(k),:)*p_k_sl';     %1*35 * 35*1
            end
        end
    end
    i
